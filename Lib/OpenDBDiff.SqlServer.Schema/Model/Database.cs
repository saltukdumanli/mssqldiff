...
namespace OpenDBDiff.SqlServer.Schema.Model
{
    public class Database : SQLServerSchemaBase, IDatabase
    {
        ...
        public  SQLScriptList ToSqlDiff(ICollection<ISchemaBase> schemas, IDatabase origin, IDatabase destination)
        {
            ...
            if (listDiff.Count > 0 && listDiff.list != null)
                foreach (var item in listDiff.list)
                {
                    try
                    {
                        item.SourceSQL = origin.Find($"[{item.SCHEMA}].[{item.OBJECTNAME}]").ToSql();
                        item.DestinationSQL = destination.Find($"[{item.SCHEMA}].[{item.OBJECTNAME}]").ToSql();

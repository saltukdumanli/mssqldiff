                    ...
                    if (isIncluded)
                    {
                        if (item.HasState(ObjectStatus.Drop))
                        {
                            ...
                            if (!item.GetWasInsertInDiffList(ScriptAction.AlterColumnFormula))
                            {
                                ...
                                list.Add(new SQLScript("ALTER TABLE " + Parent.FullName + " DROP COLUMN " + "[" + item.Name + "]" + "\r\nGO\r\n", 0, ScriptAction.AlterTable) { OBJECTNAME = Parent.Name, SCHEMA = Parent.Owner, OBJECTTYPE = ObjectType.Table, OBJECTSUBNAME = item.Name,StatusNew=ScriptActionNew.AlterTableDrop });
                            }
                            // sqlDrop += "[" + item.Name + "],";
                        }
                        if (item.HasState(ObjectStatus.Create))
                        {
                            ...
                            list.Add(new SQLScript("ALTER TABLE " + Parent.FullName + " ADD " + item.ToSql(true) + "\r\nGO\r\n", 0, ScriptAction.AlterTable) { OBJECTNAME = Parent.Name, SCHEMA = Parent.Owner, OBJECTTYPE = ObjectType.Table, OBJECTSUBNAME = item.Name, StatusNew = ScriptActionNew.AlterTableAdd });
                        }
                          //  sqlAdd += "\r\n" + item.ToSql(true) + ",";
                        ...
                        if (item.HasState(ObjectStatus.Bind))
                        {
                            if (item.Rule.Id != 0)
                            {
                                ...
                                list.Add(new SQLScript(item.Rule.ToSQLAddBind()  + "\r\nGO\r\n", 0, ScriptAction.AlterTable) { OBJECTNAME = Parent.Name, SCHEMA = Parent.Owner, OBJECTTYPE = ObjectType.Table, OBJECTSUBNAME = item.Name });
                            }
                             //   sqlBinds += item.Rule.ToSQLAddBind();
                            if (item.Rule.Id == 0)
                            {

                                list.Add(new SQLScript(item.Rule.ToSQLAddUnBind() + "\r\nGO\r\n", 0, ScriptAction.AlterTable) { OBJECTNAME = Parent.Name, SCHEMA = Parent.Owner, OBJECTTYPE = ObjectType.Table, OBJECTSUBNAME = item.Name });
                            }
